...
async function main() {
    ...
    // Querying insights with NO metric name sometimes returns all available? No.
    // Let's try querying the post with fields=insights
    const res = await fetch(`https://graph.facebook.com/v21.0/${postId}?fields=insights&access_token=${pageToken}`);
