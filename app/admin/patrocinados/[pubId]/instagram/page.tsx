...
export default function AdminLoteInstagramPage() {
  ...
  async function handlePublish() {
    try {
      ...
      const res = await fetch(`/api/admin/patrocinados/${pubId}/instagram`, {
        method: "POST",
      ...
      setSuccessMsg("Lote de Patrocinados foi publicado com sucesso no Instagram!");
    } catch(err: any) {
  ...
  if (loading) {
    return <main className="min-h-screen bg-slate-950 p-6 text-slate-400">Carregando lote administrativo...</main>;
  }
    ...
    <main className="min-h-screen bg-slate-950 p-6 md:p-12 text-white">
      <div className="mx-auto max-w-4xl">
        ...
        <h1 className="text-3xl font-bold text-pink-400 flex items-center gap-2">
           <img src="/icones/instagram.jpg" className="w-8 h-8 rounded-lg" alt="Instagram" />
           Publicar Lote: {publication?.name}
        </h1>
        <p className="mt-2 text-slate-400">
           Esta central dispara a coleção de imóveis diretamente na fila oficial do Instagram da RealStock como Carrossel.
        </p>
        ...
        {!error && !successMsg && (
          <div className="grid md:grid-cols-2 gap-8 mt-8">
            ...
            <div className="rounded-2xl border border-pink-500/30 bg-gradient-to-br from-pink-500/10 to-orange-500/10 p-6 flex flex-col justify-center text-center">
                 <Rocket className="text-pink-400 mx-auto mb-4" size={48} />
                 <h2 className="text-xl font-bold text-white mb-2">Ação Administrativa Liberada</h2>
                 <p className="text-sm text-slate-400 mb-6">Como Admin, você insere esse Carrossel no ar automaticamente, cobrindo todo o seu funil de marketing configurado.</p>
                 
                 <button 
                   onClick={handlePublish}
                   disabled={isPublishing} 
                   className="btn-primary w-full bg-gradient-to-r py-4 font-bold text-lg rounded-xl from-pink-500 to-orange-500 border-none">
                     {isPublishing ? "Criando Carrossel e Injetando Graph API..." : "Publicar Imediatamente"}
                 </button>
            </div>
          </div>
        )}
