...
export default function AdminPortfolioFacebookPage() {
  ...
  async function handlePublish() {
    try {
      ...
      const res = await fetch("/api/admin/patrocinados/publish-facebook", {
        method: "POST",
      ...
      if (!res.ok || !result.success) throw new Error(result.error || "Erro ao publicar no Facebook.");
      
      setSuccessMsg(result.message || "Carrossel Global postado com sucesso no Facebook!");
    } catch (err: any) {
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-8">
          ...
          <h1 className="mt-4 text-3xl font-bold flex items-center gap-2 text-yellow-500">
             <Gem size={28} />
             Publicar Portfólio Global no Facebook
          </h1>
          <p className="mt-2 text-slate-400">
            Ação restrita a Administradores. Cria um carrossel visual e chamativo no Feed público.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-6 h-fit">
              ...
              <p className="text-sm text-slate-400 mb-4">
                 Clique na foto para selecionar ou remover os anúncios dos diferentes usuários que irão compor o Feed.
              </p>
            ...
            <div className="rounded-2xl border border-blue-500/20 bg-gradient-to-br from-blue-500/10 to-indigo-500/5 p-6 h-fit">
              <h2 className="text-xl font-semibold mb-2">Despacho para o Facebook Feed</h2>
              <p className="text-slate-400 text-sm mb-6">
                 Diferente dos usuários, perfis administradores não pagam e saltam o gateway de pagamento.
              </p>

              {isPublishing && (
                  <div className="text-center py-6 text-blue-300 animate-pulse font-semibold">
                      Agrupando imagens e enviando pela Graph API...
                  </div>
              )}
              ...
              {!isPublishing && (
                <button
                  ...
                  className="w-full rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-4 text-center font-bold text-white transition hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed uppercase"
                >
