...
export default function InsightsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const xTotalViews = insights.x?.posts?.reduce((sum: number, p: any) => sum + (p.views || 0), 0) || 0;
    ...
    const xTotalLikes = insights.x?.posts?.reduce((sum: number, p: any) => sum + (p.likes || 0), 0) || 0;
    ...
    const xTotalComments = insights.x?.posts?.reduce((sum: number, p: any) => sum + (p.comments || 0), 0) || 0;
        ...
        <main className="min-h-screen bg-slate-950 px-6 py-8 text-white">
            <div className="mx-auto max-w-5xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {/* X (TWITTER) ORGANIC */}
                    <div className="rounded-3xl bg-gradient-to-br from-slate-800/50 to-slate-900 border border-slate-500/20 p-6 shadow-xl relative overflow-hidden">
                        <div className="flex items-center gap-3 mb-6">
                        ...
                        <p className="text-xs text-slate-400 mb-6 leading-relaxed">
                            Interações orgânicas vindas diretamente do perfil do X (Twitter) da RealStock.
                        </p>

                        <div className="flex flex-col gap-4">
                            {insights.x?.posts?.length > 0 ? insights.x.posts.map((post: any, i: number) => (
                                <div key={i} className="bg-slate-500/10 p-4 rounded-2xl backdrop-blur-sm border border-slate-500/10 flex flex-col gap-3">
                                    <div className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-1 flex items-center gap-2">
                                        <Eye size={14} className="text-slate-400" />
                                        {post.type === 'reels' ? 'Vídeo (Reels)' : 'Carrossel'}
                                    </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        ...
                                        <div>
                                            <div className="text-2xl font-black text-white">{(post.comments || 0).toLocaleString('pt-BR')}</div>
                                            <div className="text-[9px] font-bold text-slate-500 uppercase">Replies</div>
                                        </div>
                                    </div>
                                    {post.shares > 0 && (
                                        <div className="text-[10px] text-slate-400 border-t border-white/5 pt-2 flex justify-between">
                                            <span>Retweets / Quotes:</span>
