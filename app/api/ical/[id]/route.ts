...
export async function generateIcalResponse(idParam: string, isHead = false) {
  const cleanIdStr = idParam.replace(/\.ics$/i, "");
  const propertyId = Number(cleanIdStr);
  ...
  return new NextResponse(icsContent, {
    ...
    headers: {
      "Content-Type": "text/calendar; charset=utf-8",
      "Content-Disposition": `inline; filename="realstock-imovel-${propertyId}.ics"`,
      "Cache-Control": "no-cache, no-store, must-revalidate",
      "Access-Control-Allow-Origin": "*",
    },
  });
...
export async function GET(
  ...
  try {
    ...
    return await generateIcalResponse(id, false);
  } catch (error: any) {
    console.error("ERRO /api/ical/[id]:", error);
...
export async function HEAD(
  ...
  try {
    const { id } = await params;
    return await generateIcalResponse(id, true);
  } catch (error: any) {
